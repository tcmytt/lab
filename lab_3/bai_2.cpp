#include<iostream>
#include<vector>
using namespace std;

class DaThuc3 {
    private:
        int a, b, c, d;
    public:
        DaThuc3() {
            this->a = 0;
            this->b = 0;
            this->c = 0;
            this->d = 0;
        }

        friend istream& operator >> (istream& is, DaThuc3& temp) {
            cout << "\t Nhap he so bac 3: ";
            is >> temp.a;
            cout << "\t Nhap he so bac 2: ";
            is >> temp.b;
            cout << "\t Nhap he so bac 1: ";
            is >> temp.c;
            cout << "\t Nhap he so bac 0: ";
            is >> temp.d;
            return is;
        }

        friend ostream& operator << (ostream& os, DaThuc3 temp) {
            os << temp.a << "x3 + " << temp.b << "x2 + " << temp.c << "x + " << temp.d;
            return os;
        }

        DaThuc3 total(DaThuc3 y) {
            DaThuc3 temp;
            temp.a = this->a + y.a;
            temp.b = this->b + y.b;
            temp.c = this->c + y.c;
            temp.d = this->d + y.d;
            return temp;
        }

        DaThuc3 sub(DaThuc3 y) {
            DaThuc3 temp;
            temp.a = this->a - y.a;
            temp.b = this->b - y.b;
            temp.c = this->c - y.c;
            temp.d = this->d - y.d;
            return temp;
        }
};

int main() {
    vector<DaThuc3> a;
    for(int i = 0; i < 2; i++) {
        cout << "Nhap he so da thuc thu " << i + 1 << " la: " << endl;
        DaThuc3 daThuc3;
        cin >> daThuc3;
        a.push_back(daThuc3);
    }
    DaThuc3 tong = a[0].total(a[1]);
    DaThuc3 hieu = a[0].sub(a[1]);
    cout << "Tong cua hai da thuc la: " << tong << endl;
    cout << "Hieu cua hai da thuc la: " << hieu << endl;
    return 0;
}